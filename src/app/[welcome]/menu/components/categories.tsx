    ...
    <ScrollArea className="w-full">
      <div className="flex w-max space-x-4 px-4 pt-0">
        {coffee.menuCategories.map(category => 
          <Button key={category.id} variant="secondary" size="sm" className="rounded-full border opacity-20 border-black p-4 hover:opacity-100 hover:bg-[--primary] hover:text-[--secondary] hover:border-none">
            {category.name} 
          </Button>
        )}
      </div>
